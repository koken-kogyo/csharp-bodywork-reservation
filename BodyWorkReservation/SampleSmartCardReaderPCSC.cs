...
{
    public partial class SampleBodyWorkReservationPCSC : Form
    {
        private int _selectedIndex = -1;

        public SampleBodyWorkReservationPCSC()
        {
            InitializeComponent();
        }

        private void ComboBoxCardReader_SelectedIndexChanged(object sender, EventArgs e)
        {
            _selectedIndex = comboBoxCardReader.SelectedIndex;
        }

        private void ButtonCheckReader_Click(object sender, EventArgs e)
        {
            textBox_Log.Clear(); //①
            comboBoxCardReader.Items.Clear(); //②
            ...
            using (var ctx = ContextFactory.Instance.Establish(SCardScope.User))
            {
                ...
                textBox_Log.Text += @"ATR: { " + BitConverter.ToString(status.GetAtr() ?? []) + "}\r\n";
                textBox_Log.Text += @"ATR: { " + NoBarATR(status.GetAtr()) + "}\r\n";

                comboBoxCardReader.Items.AddRange(status.GetReaderNames());//⑤
            }
            textBox_Log.Text += "-----------------------------------------------\r\n"; //⑥
        }
        ...
        /// <returns></returns>
        private static string NoBarATR(byte[] arr)
        {
        ...
        private static bool NoReaderFound(string[] readerNames)
        {
            if (readerNames == null) return true;
            if (readerNames.Length == 0) return true;
        ...
        private void ButtonReadCardType_Click(object sender, EventArgs e)
        {
            var contextFactory = ContextFactory.Instance;
        ...
        private void ButtonSuica090F_Click(object sender, EventArgs e)
        {
            var contextFactory = ContextFactory.Instance;
